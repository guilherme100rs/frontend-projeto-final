...
function CartProvider({ children }) {
    const [cart, setCart] = useState( JSON.parse(localStorage.getItem(`@guilherme100:cart`)) || [])
    ...
    async function handleResetCart() {
        localStorage.removeItem(`@guilherme100:cart`);
        setCart([]);
    }

    useEffect(() => {
        localStorage.setItem(`@guilherme100:cart`, JSON.stringify(cart));
    }, [cart])
